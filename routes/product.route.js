...
route.post("/carrinho/:iduser", async(req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(iduser, { cart: createdCart.id }, { new: true })
...
route.patch("/carrinho/:idcart/product/:idproduct", async(req, res) => {
    try {
        ...
        const updatedCart = await Cart.findByIdAndUpdate(idcart, { $push: { products: idproduct } }, { new: true })
...
route.patch("/carrinho/:idcart/products/:idproduct/delete", async(req, res) => {
    try {
        const { idcart, idproduct } = req.params;
